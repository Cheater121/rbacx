...
import time, json, statistics, argparse
...
def gen_policy(n: int) -> dict:
    rules = []
    for i in range(n-1):
        rules.append({"id": f"permit_{i}", "effect":"permit", "actions":["read"], "resource":{"type":"doc","attrs":{"k":i}},
                      "condition":{"==":[{"attr":"resource.attrs.k"}, i]}})
    rules.append({"id":"deny_other","effect":"deny","actions":["read"],"resource":{"type":"doc"}})
...
def run(size: int, iters: int):
    ...
    return {"p50": statistics.median(lat), "avg": sum(lat)/len(lat), "p90": percentile(lat, 90), "allowed": d.allowed}
