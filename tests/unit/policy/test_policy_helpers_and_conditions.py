...
def test_match_resource_rtype_non_str_non_list_falls_into_else_branch():
    # Covers the else branch in match_resource where r_type is neither str nor
    # list (e.g. an int). The value is wrapped in a single-element list and
    # matched via str() coercion in lax mode — so int 1 vs int 1 matches,
    # but int 1 vs int 2 does not.
    assert policy.match_resource({"type": 1}, {"type": 1}) is True
    assert policy.match_resource({"type": 1}, {"type": 2}) is False
