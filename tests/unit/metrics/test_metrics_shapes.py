...
def test_prometheus_sink_importable():
    ...
    m = None
    try:
        m = PrometheusMetrics(namespace="rbacx_test")  # may raise if no client
    except RuntimeError:
        pytest.skip("prometheus_client not installed")
    # inc shouldn't raise
    m.inc("rbacx_decision_total", labels={"allowed":"true","reason":"ok"})
...
def test_otlp_sink_importable():
    ...
    try:
        m = OpenTelemetryMetrics(meter_name="rbacx_test")
    except RuntimeError:
        pytest.skip("opentelemetry-api not installed")
    m.inc("rbacx_decision_total", labels={"allowed":"true","reason":"ok"})
