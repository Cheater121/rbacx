...
def test_context_plain_dict_resolution_and_terms_accept_branch():
    """
    Exercise context resolution when `context` is a plain dict (no `.attrs`)
    and cover `require_terms_accept` success & failure.
    """
...
def test_obligation_with_invalid_on_value_is_ignored_and_baseline_is_returned():
    """
    Cover the `on` filter ignoring invalid values (not in {"permit","deny"}).
...
def test_require_level_invalid_min_uses_zero_and_fails_when_auth_level_below_zero():
    """
    Covers numeric parse error for attrs['min'] -> except -> min_level = 0 (lines 91-92).
...
def test_require_reauth_invalid_max_age_uses_zero_and_fails_when_age_gt_zero():
    """
    Covers numeric parse error for attrs['max_age'] -> except -> max_age = 0 (lines 128-130).
...
def test_consent_without_key_truthy_falsy_and_missing():
    """
    Covers consent branch when no 'attrs.key' is provided (lines 108-109):
    - truthy consent -> ok
    - falsy or missing consent -> (False, "consent")
    """
    chk = BasicObligationChecker()
    dec = _modern("permit", [{"on": "permit", "type": "require_consent"}])

    ok_true, ch_true = chk.check(dec, {"consent": True})
    assert (ok_true, ch_true) == (True, None)

    ok_false, ch_false = chk.check(dec, {"consent": False})
    assert (ok_false, ch_false) == (False, "consent")

    ok_missing, ch_missing = chk.check(dec, {})
    assert (ok_missing, ch_missing) == (False, "consent")
