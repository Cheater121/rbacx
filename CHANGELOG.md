...
* `engine.py` / `Guard.evaluate_sync()`: the `ThreadPoolExecutor` used when
  `evaluate_sync()` is called from inside a running event loop is now a
  class-level attribute (`Guard._executor`) created lazily on first use and
  reused across all subsequent calls. Previously a new one-thread pool was
  created and torn down on every invocation, adding measurable overhead in
  sync-heavy workloads such as Django middleware.

* `engine.py` / `Guard.__init__()`: removed the block that manipulated the
  global asyncio event loop (`asyncio.get_event_loop()` /
  `asyncio.new_event_loop()` / `asyncio.set_event_loop()`). This side-effect
  was fragile and could interfere with async frameworks (e.g. Django
  `async_to_sync`). `evaluate_sync()` has always been safe to call without it,
  and the removal has no effect on behaviour.
